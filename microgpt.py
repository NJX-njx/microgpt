...
class Value:
    ...
    def __truediv__(self, other): # direct div: fewer graph nodes than self * other**-1
        other = other if isinstance(other, Value) else Value(other)
        out = Value(self.data / other.data, (self, other), '/')
        def _backward():
    ...
    def backward(self): # iterative topological sort (no recursion limit)
        topo = []
        visited = set()
        stack = [(self, False)]
        while stack:
    ...
    def __rtruediv__(self, other): # uses direct __truediv__ now
        other = other if isinstance(other, Value) else Value(other)
        return other / self
...
def cross_entropy(logits, target_id): # fused log-softmax + nll: fewer nodes, numerically stable
    max_val = max(val.data for val in logits)
...
for step in range(num_steps):
    ...
    for pos_id in range(n):
        ...
        loss_t = cross_entropy(logits, target_id) # fused cross-entropy replaces softmax+log
        losses.append(loss_t)
    loss = (1 / n) * sum(losses[1:], losses[0]) # avoid phantom Value(0) node
